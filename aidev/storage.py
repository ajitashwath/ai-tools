...
_COLUMNS = (
    "id, name, parent_id, start_time, end_time, status, metadata, errors, "
    "inputs, outputs, model, model_token_count, operation, ttft, "
    "tokens_per_sec, stop_reason, total_tokens"
)
...
def _row_to_span(row: tuple) -> Span:
    ...
    instead of hand-building ``Span(...)`` objects.
    """
    return Span(
        id=row[0],
        name=row[1],
...
class TraceSQLite:
    ...
    A single connection is shared across threads (FastAPI serves concurrent
    requests on a threadpool while ``check_same_thread=False`` is required),
    so every database access is serialized with a lock. WAL journal mode is
    enabled so readers don't block writers.
    """
    ...
    def get_by_id(self, span_id: str) -> Span | None:
        ...
        with self._lock:
            row = self._conn.execute(
                f"SELECT {_COLUMNS} FROM spans WHERE id = ?",
                (span_id,),
            ).fetchone()
        if row is None:
            return None
    ...
    def get_root_spans(self) -> list[Span]:
        ...
        with self._lock:
            rows = self._conn.execute(
                f"SELECT {_COLUMNS} FROM spans WHERE parent_id IS NULL ORDER BY start_time"
            ).fetchall()
    ...
    def get_all_spans(self) -> list[Span]:
        ...
        with self._lock:
            rows = self._conn.execute(
                f"SELECT {_COLUMNS} FROM spans ORDER BY start_time"
            ).fetchall()
        return [_row_to_span(row) for row in rows]
    ...
    def get_children(self, parent_id: str) -> list[Span]:
        ...
        with self._lock:
            rows = self._conn.execute(
                f"SELECT {_COLUMNS} FROM spans WHERE parent_id = ? ORDER BY start_time",
                (parent_id,),
            ).fetchall()
        return [_row_to_span(row) for row in rows]
